...
define(function(require) {
    ...
    Interface.ensureImplements = function(instance, instanceType) {
        console.log(
            'Interface ensureImplements called with: ' + instanceType.name
        );
        ...
        if (arguments.length < 2) {
            throw new Error(
                arguments.length + ' arguments passed, but expected 2.'
            );
        }
        ...
        for (var i = 1, len = arguments.length; i < len; i++) {
            ...
            for (
                var j = 0, methodsLen = objectToTest.methods.length;
                j < methodsLen;
                j++
            ) {

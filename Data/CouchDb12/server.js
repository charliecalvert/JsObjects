...
app.get('/attachPng', function(request, response) {
    'use strict';
 ...
 */
var doInsert = function(rev, response, key, docName) {
    var fs = require('fs');
...
app.get('/attachHtml', function(request, response) {
    'use strict';
    ...
    doInsert(null, response, request.query.key, request.query.doc);
});

app.get('/getAttachedHtml', function(request, response) {
    'use strict';
    console.log('/getAttachedHtml called');
    console.log(request.query);

    var nanoDb = nano.db.use(dbName);
    nanoDb.attachment.get(request.query.key, request.query.doc, function(
        err,
        body
    ) {
        if (!err) {
...
app.get('/attachUpdateHtml', function(request, response) {
    'use strict';
    ...
    nanoDb.get('attachMe', function(error, existing) {
        if (!error) {
            ...
            doInsert({ rev: existing._rev }, response, key, doc);
        } else {
            console.log(error);

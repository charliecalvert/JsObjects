    ...
    QueryMongo.prototype.getData = function(response) {
        ...
        MongoClient.connect(
            ...
            function(err, client) {
                ...
                insertIntoCollection(database, collectionName, {
                    firstName: 'Suzy'
                });
                getCollection(database, collectionName, function(testInsert) {
                    var body = '<html><body>';
                    body +=
                        '<h2>Mongo Data: ' + testInsert[0].firstName + '</h2>';
                    body +=
                        '<p>This HTML is hardcoded into Server01.js. See the getCollection method.</p></body></html>';

...
app.get('/getitem', function(request, result) {
	...
	sdb.getItem('History', request.query.itemName, function(error, getItemResult) {
		if (error) {
			console.log(error);
		} else {
...
function getItemsToDelete(result) {
	sdb.select('select ItemName from History where Category="' + coreData.Category + '"', function(error, queryResult, metadata) {
		if (error) {
			result.send({
